...
fn b(s: &str) -> SharedByte {
    SharedByte::from_str(s)
}

fn bv(items: &[&str]) -> Vec<SharedByte> {
    items.iter().map(|s| b(s)).collect()
}

fn fv(pairs: &[(&str, &str)]) -> Vec<(SharedByte, SharedByte)> {
    pairs.iter().map(|(f, v)| (b(f), b(v))).collect()
}

fn sm(pairs: &[(&str, f64)]) -> Vec<(f64, SharedByte)> {
...
fn hash_hset_hget_basic() {
    ...
    let added = art
        .cmd_hset(b"key", &fv(&[("f1", "v1"), ("f2", "v2")]), None)
        .unwrap();
    assert_eq!(added, 2);
...
fn hash_hset_mixed_add_update() {
    ...
    let added = art
        .cmd_hset(b"k", &fv(&[("exists", "new"), ("fresh", "v2")]), None)
        .unwrap();
    assert_eq!(added, 1);
}
...
fn hash_hgetall_order() {
    let mut art = OxidArt::new();
    art.cmd_hset(b"k", &fv(&[("z", "1"), ("a", "2"), ("m", "3")]), None)
        .unwrap();
...
fn hash_hdel_basic() {
    let mut art = OxidArt::new();
    art.cmd_hset(b"k", &fv(&[("a", "1"), ("b", "2")]), None)
        .unwrap();
...
fn hash_hincrby_basic() {
    ...
    let v = art
        .cmd_hincrby(b"k", SharedByte::from_str("counter"), 5)
        .unwrap();
    assert_eq!(v, 5);
    let v2 = art
        .cmd_hincrby(b"k", SharedByte::from_str("counter"), -3)
        .unwrap();
    assert_eq!(v2, 2);
}
...
fn hash_hincrby_non_numeric_field_errors() {
    let mut art = OxidArt::new();
    art.cmd_hset(b"k", &fv(&[("f", "notanumber")]), None)
        .unwrap();
...
fn hash_many_similar_keys_isolation() {
    ...
    for i in 0u32..50 {
        ...
        art.cmd_hset(key.as_bytes(), &fv(&[(&field, &value)]), None)
            .unwrap();
    }
    ...
    for i in 0u32..50 {
        ...
        // Length is exactly 1
        assert_eq!(
            art.cmd_hlen(key.as_bytes()).unwrap(),
            1,
            "wrong len for user:{i}"
        );
    }
}
...
fn hash_delete_one_preserves_siblings() {
    ...
    for i in 0u32..10 {
        let key = format!("h:{i}");
        art.cmd_hset(key.as_bytes(), &fv(&[("f", &i.to_string())]), None)
            .unwrap();
    }
...
fn set_smembers_sorted() {
    ...
    art.cmd_sadd(b"s", &bv(&["z", "m", "a", "b"]), None)
        .unwrap();
...
fn zset_double_index_consistency_after_updates() {
    ...
    for chunk in range.chunks(2) {
        ...
        let via_zscore = art.cmd_zscore(b"z", chunk[0].clone()).unwrap().unwrap();
        assert!(
            (via_zscore - score).abs() < 1e-9,
            "ZSCORE and ZRANGE disagree"
        );
        prev_score = score;
    }
}
...
fn zset_equal_scores_lexicographic_tiebreak() {
    ...
    art.cmd_zadd(b("z"), &sm(&[("z", 1.0), ("a", 1.0), ("m", 1.0)]), None)
        .unwrap();
    let r = art.cmd_zrange(b"z", 0, -1, false).unwrap();
...
fn zset_score_update_removes_old_sorted_entry() {
    let mut art = OxidArt::new();
    art.cmd_zadd(b("z"), &sm(&[("a", 1.0), ("b", 1.0), ("c", 1.0)]), None)
        .unwrap();
...
fn zset_many_similar_keys_isolation() {
    ...
    for i in 0u32..50 {
        let key = format!("zs:{i}");
        let member = format!("m{i}");
        let score = art.cmd_zscore(key.as_bytes(), b(&member)).unwrap();
        assert_eq!(score, Some(i as f64), "wrong score for zs:{i}");
        assert_eq!(
            art.cmd_zcard(key.as_bytes()).unwrap(),
            1,
            "wrong card for zs:{i}"
        );
    }
...
fn zset_delete_one_preserves_siblings() {
    ...
    for i in 0u32..10 {
        let key = format!("z:{i}");
        art.cmd_zadd(b(&key), &sm(&[("m", i as f64)]), None)
            .unwrap();
    }
    ...
    for i in 0u32..10 {
        ...
        assert_eq!(
            art.cmd_zscore(key.as_bytes(), b("m")).unwrap(),
            Some(i as f64),
            "z:{i} score corrupted"
        );
    }
...
fn cross_type_similar_keys_no_bleed() {
    ...
    art.cmd_hset(b"user:1", &fv(&[("name", "Alice")]), None)
        .unwrap();
    art.cmd_sadd(b"user:10", &bv(&["tag1", "tag2"]), None)
        .unwrap();
    art.cmd_zadd(b("user:100"), &sm(&[("score", 42.0)]), None)
        .unwrap();
    ...
    assert!(
        art.cmd_hget(b"user:10", b"name").is_err(),
        "set key must reject hget"
    );
    assert!(
        art.cmd_scard(b"user:1").is_err(),
        "hash key must reject scard"
    );
    assert!(
        art.cmd_zcard(b"user:10").is_err(),
        "set key must reject zcard"
    );
}
...
fn cross_type_set_overwrites_with_string() {
    ...
    art.set(SharedByte::from_str("k"), Value::from_str("stringnow"));

    assert!(
        art.cmd_hget(b"k", b"f").is_err(),
        "k must now be string, not hash"
    );
}
...
fn cross_type_mixed_tree_deep() {
    ...
    for i in 0u32..20 {
        ...
        assert_eq!(
            art.cmd_hget(hk.as_bytes(), b"n").unwrap(),
            Some(b(&is)),
            "hash prefix:h:{i}"
        );
        assert!(
            art.cmd_sismember(sk.as_bytes(), b(&is)).unwrap(),
            "set prefix:s:{i}"
        );
        assert_eq!(
            art.cmd_zscore(zk.as_bytes(), b("m")).unwrap(),
            Some(i as f64),
            "zset prefix:z:{i}"
        );
    }
...
fn hash_add_delete_cycle_many_keys() {
    ...
    for i in 0u32..30 {
        let k = format!("k:{i}");
        art.cmd_hset(k.as_bytes(), &fv(&[("f", &i.to_string())]), None)
            .unwrap();
    }
    // Delete odd keys
...
fn set_add_delete_cycle_many_keys() {
    ...
    for i in (1u32..30).step_by(2) {
        let k = format!("k:{i}");
        art.cmd_srem(k.as_bytes(), &[b(&i.to_string())]).unwrap();
    }
    for i in (0u32..30).step_by(2) {
        let k = format!("k:{i}");
        assert!(
            art.cmd_sismember(k.as_bytes(), b(&i.to_string())).unwrap(),
            "k:{i} corrupted"
        );
    }
}
...
fn zset_add_delete_cycle_many_keys() {
    ...
    for i in 0u32..30 {
        let k = format!("k:{i}");
        art.cmd_zadd(b(&k), &[(i as f64, b("m"))], None).unwrap();
    }
    for i in (1u32..30).step_by(2) {
        let k = format!("k:{i}");
    ...
    for i in (0u32..30).step_by(2) {
        let k = format!("k:{i}");
        assert_eq!(
            art.cmd_zscore(k.as_bytes(), b("m")).unwrap(),
            Some(i as f64),
            "k:{i} corrupted"
        );
    }
